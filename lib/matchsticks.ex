defmodule Matchsticks do
  ...
  ## Example
      ...
      12
  """
  def decoded_diff(file) do
    Regex.scan(~r/(".*")\n/, file, capture: :all_but_first)
    |> List.flatten
    |> Enum.map(&decode/1)
    |> Enum.map(&lengths/1)
    |> Enum.reduce(0, fn({i, o}, acc) ->
      acc + i - o
    end)
  end
...
(2 + 5 + 10 + 6 = 23)
  ...
  """
  def encode(input) do
    output = "\"#{do_encode(input |> to_char_list, [])}\""
    {output, input}
  end

  def do_encode([], output), do: output
  def do_encode([h | t], output) do
    case h in ?0..?9 || h in ?a..?z do
      true -> do_encode(t, output ++ [h])
      _ -> do_encode(t, output ++ [?\\, h])
    end
  ...
  ## Example
      ...
      iex> "\\"abc\\"" |> decode |> lengths
      {5, 3}

      iex> "\\"aaa\\\\\\"aaa\\"" |> decode |> lengths
      {10, 7}

      iex> "\\"\\\\x27\\"" |> decode |> lengths
      {6, 1}
  """
  def lengths({input, output}) do
    [input_length, output_length] = [input, output]
    |> Enum.map(fn(i) ->
      i |> to_char_list |> Enum.count
    end)
